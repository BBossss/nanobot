...
- 支持在排障过程中输出稳定、可解释的进度反馈
- 支持沉淀排障案例，并可复用历史案例
- 支持定时巡检与定期报告
- 支持 CLI 与 Mattermost 作为交互入口
- 保持最小权限和高可审计性

V1 不实现任意跨主机执行，不开放高危变更操作；但可在用户明确确认后，对受限只读动作执行多节点调查。
...
当前 V1 正式能力仍以本机/单节点排障为主，不将通用跨主机执行视为已完成平台能力。

但考虑到现场排障存在“HCIGuard 跑在内网机器上，再远程排查其他 HCI 主机”的现实需求，V1.5 可接受一层临时桥接方案：

- 通过 `ssh-copy-id` 等方式预先建立免密 SSH
- 由现有执行层临时承载远程只读命令
- Skill 只负责约束远程排障规则，不新增独立远程 Skill

该桥接层的定位是：

- 解决短期真实使用需求
- 不阻塞日常现场排障
- 为后续正式 `RemoteExecutor` 设计留出演进空间

边界要求：

- 不在聊天、案例、历史记录中存储明文密码
- 不把裸 `ssh` 白名单视为最终远程平台设计
- 仅允许只读远程动作
- 审计中应至少保留远程命令痕迹和目标主机信息
- 如扩展到多节点调查，必须先经过候选范围确认
...
在 `AgentLoop` 现有 progress callback / bus 通路上补充一层轻量反馈渲染，用于提升排障过程可见性，而不新增新的传输层。
...
该层应保持程序化、可测、边界稳定，不依赖模型自由生成整段过程播报。
...
对于远程只读桥接场景，建议继续沿用这条边界：

- 主代码 / 执行层负责“是否能远程执行”
- Skill 负责“远程排障时应如何采证、如何标注来源、如何控制风险”
...
- “多节点部署编排”不包括当前已收口的受限多节点只读调查
- 当前多节点能力应理解为排障流程中的受控扩查，而不是独立平台编排层
