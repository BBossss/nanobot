...
from nanobot.agent.workflow import feedback as workflow_feedback
...
class InvestigationState:
    ...
    visited_buckets: list[str] = field(default_factory=list)
...
class AgentLoop:
            ...
            if response.has_tool_calls:
                ...
                for tool_call in response.tool_calls:
                    ...
                    if object_signature and object_signature in object_result_cache:
                        ...
                        result = await self._execute_tool_call_with_feedback(
                            tool_call.name,
                            tool_call.arguments,
                            session=session,
                ...
                if on_progress:
                    ...
                    summary = self._render_progress_summary(investigation)
                    if summary:
    ...
    async def _execute_tool_call_with_feedback(
        self,
        name: str,
        arguments: dict[str, Any],
        *,
        ...
        multi_target_total = self._resolved_multi_target_total(name, arguments, session=session)
        if multi_target_total > 1:
            investigation.multi_target_active = True

        if on_progress:
            reason = self._render_progress_reason(
                bucket,
                previous_bucket=investigation.last_bucket,
                multi_target_total=multi_target_total,
            )
            if reason:
                await on_progress(reason)
            await on_progress(
                self._render_progress_action(
                    name,
                    arguments,
                    multi_target_total=multi_target_total,
                )
        ...
        result = await self._await_tool_with_heartbeat(
            name,
            arguments,
            session=session,
            on_progress=on_progress,
        ...
        investigation.last_bucket = bucket
        return result

    async def _await_tool_with_heartbeat(
        self,
        name: str,
        arguments: dict[str, Any],
        *,
        ...
        async def _on_target_progress(completed: int, total: int, _target_id: str) -> None:
            if on_progress:
                await on_progress(f"仍在多节点检查中，已完成 {completed}/{total}。")
    ...
    def _tool_feedback_bucket(name: str) -> str:
        if name in {"read_log_tail", "search_log", "find_logs", "journal_tail", "diagnose_log_read", "diagnose_log_search"}:
            return "log"
        if name in {"service_status", "diagnose_system_status"}:
            return "service"
        if name in {"process_snapshot"}:
            return "process"
        if name in {"disk_snapshot"}:
            return "disk"
        if name in {"network_snapshot"}:
            return "network"
        return "other"

    def _resolved_multi_target_total(
        self,
        name: str,
        arguments: dict[str, Any],
        *,
    ...
    def _render_progress_reason(
        ...
        *,
        ...
        return workflow_feedback.render_progress_reason(
            bucket,
    ...
    @staticmethod
    def _render_multi_target_reason(bucket: str) -> str:
        return workflow_feedback.render_multi_target_reason(bucket)

    @staticmethod
    def _render_progress_action(name: str, arguments: dict[str, Any], *, multi_target_total: int) -> str:
        return workflow_feedback.render_progress_action(
            name,
            arguments,
            multi_target_total=multi_target_total,
        )
    ...
    @staticmethod
    def _render_progress_heartbeat(name: str, *, multi_target_total: int) -> str:
        return workflow_feedback.render_progress_heartbeat(
            name,
            multi_target_total=multi_target_total,
        )

    @staticmethod
    def _render_progress_summary(investigation: InvestigationState) -> str | None:
        return workflow_feedback.render_progress_summary(investigation)
